...
書籍のレビューや評価をユーザー同士で共有し、読書体験をより充実させることを目的としたWebアプリケーションです。

書籍の登録・レビュー投稿・お気に入り・ランキングなどの機能に加え、読書計画や通知機能を実装することで、読みたい本の管理から読書後の記録までを一つのアプリケーションで行えるようにしています。

また、公開APIを提供することで、外部アプリケーションからも書籍情報の取得・登録・更新・削除を行える構成としています。
...
※ APIキーそのものはGitHubなどの公開リポジトリには登録しないでください。
...
`.env` のデータベース接続情報と、前の手順で取得したGoogle Books APIキーを以下のように設定します。
